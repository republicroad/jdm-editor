 ...
 */
export function markdownCodeProps(props: { children?: ReactNode; className?: string }): {
  code: string;
  language?: string;
} {
...
export function markdownFences(markdown: string): CodeBlockMarkdownPart[] {
  ...
  for (const line of lines) {
    ...
    if (fence && inFence && fence[1][0] === opener[0] && fence[1].length >= opener.length && !fence[2]) {
      flushCode(false);
      inFence = false;
      continue;
    }
...
const ansiVar = (slot: number, fallback: string) => `var(--code-ansi-${ANSI_SLOTS[slot]}, ${fallback})`;
    ...
    while ((match = SGR_RE.exec(raw))) {
      ...
      for (let i = 0; i < params.length; i += 1) {
        ...
        if (code === 0) {
          ...
          style.color = style.colorDark = hex;
          i += 4;
        } else if (code === 48 && (params[i + 1] === 5 || params[i + 1] === 2)) {
          i += params[i + 1] === 5 ? 2 : 4;
        }
