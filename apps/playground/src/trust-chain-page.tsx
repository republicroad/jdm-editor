...
/**
 * Trust Chain 页（DD 系列）：playground 第六页签——
 * 执行（含审计事件）→ 确定性回放 → 影子对比，把"声明→强制→证据→重演"信任链变成可演示的界面。
...
export const TrustChainPage: React.FC<{ model: unknown }> = ({ model }) => {
  ...
  const onReplay = useCallback(async () => {
    ...
    if (!res.ok) {
      setReplayError(
        `replay ${res.status}: ${(res.json as { error?: string; details?: string })?.error ?? 'failed'}${(res.json as { details?: string })?.details ? ' — ' + (res.json as { details?: string }).details : ''}`,
      );
      setReplaying(false);
      return;
    }
  ...
  const onShadowCompare = useCallback(async () => {
    ...
    if (!res.ok) {
      setShadowError(`shadow ${res.status}: ${(res.json as { error?: string; details?: string })?.error ?? 'failed'}`);
      setShadowing(false);
      return;
    }
  ...
  return (
    <div style={{ padding: '24px 16px', maxWidth: 900, margin: '0 auto', display: 'grid', gap: 20 }}>
      <h2 style={{ margin: 0, fontSize: 18 }}>Trust Chain — 声明 → 强制 → 证据 → 重演</h2>
