...
function useShallowStable<V>(value: V, equal: (a: V & object, b: V & object) => boolean): V {
  ...
  const comparable = typeof value === 'object' && value !== null && typeof stable === 'object' && stable !== null;
...
export function useCascaderSelection<T = unknown>(): CascaderSelection<T> {
  ...
  return React.useMemo(() => {
    const selected = selectedValues.map((value) => index.byValue.get(value)).filter(Boolean) as CascaderNode<T>[];
...
interface CascaderLatest<T> {
  ...
  onValueChange: ((value: string | string[], details: CascaderChangeDetails<T>) => void) | undefined;
...
function Cascader<T>({
  ...
  useCascaderDevWarnings({
    ...
    value: valueProp,
    defaultValue,
    hasExpanded: expandedProp !== undefined || defaultExpanded !== undefined || onExpandedChange !== undefined,
  ...
  const virtualized =
    hasVirtualRenderer && (virtualLatch.key === levelKey ? virtualLatch.on || wantsVirtual : wantsVirtual);
   ...
   */
  const [pendingNavigation, setPendingNavigation] = React.useState<CascaderPendingNavigation | null>(null);
  ...
  const isBranch = React.useCallback((node: CascaderNode<T>) => isCascaderBranch(index, node), [index]);
  ...
  const resolvedActions = React.useMemo<CascaderActionItem[]>(() => stableActions ?? EMPTY_ACTIONS, [stableActions]);

  // A Set, not an `includes` scan: `isSelected` runs once per rendered row.
  const selectedSet = React.useMemo(() => new Set(selectedValues), [selectedValues]);

  const isSelected = React.useCallback((node: CascaderNode<T>) => selectedSet.has(node.value), [selectedSet]);
  ...
  const announcementTarget = React.useMemo(() => {
    ...
    const toggled =
      mode === 'tree' && announceSignal.toggled ? (index.byValue.get(announceSignal.toggled) ?? null) : null;
    ...
    const pendingState = pendingNavigation ? (loadStates.get(pendingNavigation.value) ?? null) : null;
    ...
    if (pendingState?.error) {
      ...
      text = query.trim() ? labels.searchingAnnouncement : total > 0 ? labels.loadingMore : labels.loading;
    } else if (query.trim()) {
  ...
  const emitSelection = React.useCallback(
    (nextValues: string[], node: CascaderNode<T> | null, reason: CascaderChangeReason) => {
      ...
      const unchanged = nextValues.length === current.length && nextValues.every((entry, i) => entry === current[i]);
      if (unchanged) return;
  ...
  const commit = React.useCallback(
    (node: CascaderNode<T>) => {
      ...
      if (currentCascade) {
        ...
        if (selecting && currentMax != null && next.length > currentMax && next.length > current.length) {
        ...
        if (swept > 0) {
          announceNotice(currentLabels.cascadeAnnouncement(node.label ?? node.value, swept, selecting));
        }
        setQuery('');
        return;
      }
  ...
  const handleComboboxValueChange = React.useCallback(
    (next: CascaderNode<T> | CascaderNode<T>[] | null, details: ComboboxPrimitive.Root.ChangeEventDetails) => {
      if (!multiple) {
    ...
    <CascaderActionsContext.Provider value={actionsValue as unknown as CascaderActionsContextValue}>
      <CascaderStateContext.Provider value={stateValue as unknown as CascaderStateContextValue}>
        <CascaderHighlightContext.Provider value={highlightStore}>
          <CascaderRenderContext.Provider value={renderContext as unknown as CascaderRenderContextValue}>
...
function CascaderTrigger({ className, children, showIcon = true, ref, ...props }: CascaderTriggerProps) {
  ...
  React.useEffect(() => {
    ...
    const named =
      ...
      (element.id !== '' && element.ownerDocument.querySelector(`label[for="${CSS.escape(element.id)}"]`) !== null);
    if (named) return;
    warnCascaderOnce(
      'trigger-unnamed',
  ...
  return (
    ...
    >
      {children}
      {showIcon ? <ChevronDownIcon className={`${TRIGGER_ICON_CLASS} pointer-events-none shrink-0`} /> : null}
    </ComboboxPrimitive.Trigger>
  );
}
 ...
 * ```
 */
function CascaderChips({ className, children, placeholder, strategy = 'all', ...props }: CascaderChipsProps) {
  const { labels, multiple, invalid, resolveNode, index, isSelectable, setSelection } = useCascaderActions();
  ...
  const removeClosure = (node: CascaderNode) => {
    setSelection(applyCascadeSelection(index, selectedValues, node.value, false, isSelectable), 'deselect');
  };
...
type CascaderChipRemoveClickEvent = Parameters<NonNullable<ComboboxPrimitive.ChipRemove.Props['onClick']>>[0];
...
function CascaderChip({
  ...
  const label = segments.length
    ? segments.map((segment) => (segment.type === 'node' ? segment.node.label : '…')).join(` ${labels.pathSeparator} `)
    : node.label;
...
export { createCascaderHighlightStore, useCascaderActions, useCascaderHighlight, useCascaderRender, useCascaderState };
