...
function sameLoadState(a: CascaderLoadState, b: CascaderLoadState): boolean {
  return a.loading === b.loading && a.error === b.error && a.hasMore === b.hasMore && a.cursor === b.cursor;
}
...
function withDetached<T>(store: CascaderLoaderStore<T>, items: readonly CascaderNode<T>[]): CascaderLoaderStore<T> {
 ...
 */
function withChain<T>(store: CascaderLoaderStore<T>, chain: readonly CascaderNode<T>[]): CascaderLoaderStore<T> {
...
export function useCascaderLoader<T = unknown>({
  ...
  const runLoad = React.useCallback(
    (key: string, reason: CascaderLoadReason, cursor: string | undefined, append: boolean) => {
      ...
      const node = key === CASCADER_ROOT_KEY ? null : (currentBase.byValue.get(key) ?? known.current.get(key) ?? null);
      ...
      const stale = () =>
        controller.signal.aborted || startEpoch !== epoch.current || requestId !== requestIds.current.get(key);
  ...
  const runSearch = React.useCallback(
    (text: string) => {
      ...
      const stale = () =>
        controller.signal.aborted || startEpoch !== epoch.current || requestId !== requestIds.current.get(SEARCH_KEY);
  ...
  return React.useMemo(
    ...
    [hasLoader, store, searchResults, searchState, ensureLevel, loadMore, retryLevel, invalidateLevel, prefetchNode],
  );
}
