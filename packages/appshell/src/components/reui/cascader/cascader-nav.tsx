      ...
      <React.Fragment key={segment.type === 'node' ? segment.node.value : `gap-${i}`}>
        ...
        {segment.type === 'ellipsis' ? (
          ...
          <span
            ...
            className={cn('max-w-32 truncate', segment.node.value === currentValue && 'text-foreground font-medium')}
          >
            {segment.node.label}
          </span>
        )}
...
function CascaderInput({
  ...
  const { labels, popLevel, mode, getHighlighted, isBranch, navigate, index, toggleExpanded, inline, invalid, baseId } =
    useCascaderActions();
  const direction = useDirection();
  const { currentParent, query, path, renderedItems, treeRows } = useCascaderState();

  const resolvedPlaceholder = placeholder ?? resolveCascaderSearchLabel(labels, currentParent?.label);
  ...
  const moveHighlightTo = (field: HTMLInputElement, targetIndex: number) => {
    ...
    for (let step = 0; step < treeRows.length; step += 1) {
      ...
      const next = rowIndex();
      const progressed =
        key === 'ArrowUp' ? next < current && next >= targetIndex : next > current && next <= targetIndex;
  ...
  const handleKeyDown = (event: CascaderInputKeyEvent) => {
    ...
    if (event.key === 'ArrowDown' && !event.altKey) {
      ...
      const lastValue = mode === 'tree' ? treeRows[rowCount - 1]?.node.value : renderedItems[rowCount - 1]?.value;
      const highlighted = getHighlighted();
    ...
    if ((event.key === backKey && caretAtStart) || (event.key === 'Backspace' && query.length === 0)) {
      if (path.length > 0) {
        event.preventDefault();
        popLevel();
      }
