...
function useCascaderVirtualGutter(contentElement: HTMLElement | null): CascaderVirtualGutter {
  ...
  React.useLayoutEffect(() => {
    ...
    setGutter((previous) => (previous.block === block && previous.inline === inline ? previous : { block, inline }));
  }, [contentElement]);
...
function cascaderVirtualRowStyle(start: number, gutter: CascaderVirtualGutter): React.CSSProperties {
  return {
    position: 'absolute',
    top: 0,
...
function CascaderVirtualColumn({ column, estimateSize, overscan }: CascaderVirtualColumnProps) {
  const { virtualized, registerVirtualRenderer, virtualize, virtualizeThreshold } = useCascaderActions();

  React.useLayoutEffect(() => registerVirtualRenderer(), [registerVirtualRenderer]);

  const windowed = column.active ? virtualized : (virtualize ?? column.items.length >= virtualizeThreshold);
  ...
  return (
    <CascaderColumnPanel column={column} virtualized>
      {column.active ? (
        ...
        <CascaderVirtualColumnRows column={column} estimateSize={estimateSize} overscan={overscan} activeIndex={-1} />
      )}
    </CascaderColumnPanel>
  );
}
