 ...
 */
export const CASCADER_LIST_HEIGHT_CLASS = 'max-h-[min(var(--available-height,100vh),var(--cascader-max-height,24rem))]';
...
export function getCascaderTabTarget(panel: HTMLElement, from: Element | null, backwards: boolean): HTMLElement | null {
 ...
 */
export function createCascaderMoreNode<T = unknown>(parentKey: string, loadedCount = 0): CascaderNode<T> {
  return {
...
export function buildCascaderIndex<T = unknown>(
  ...
  if (getParent) {
    ...
    const walk = (nodes: readonly CascaderNode<T>[] | undefined, parent: string | null, depth: number) => {
...
export function getCascaderChildren<T>(index: CascaderIndex<T>, parent?: string | null): CascaderNode<T>[] {
...
export function getCascaderPath<T>(index: CascaderIndex<T>, value: string | null | undefined): CascaderNode<T>[] {
...
export function collapseCascaderPath<T>(
  ...
  if (collapse === 'start') {
    ...
    return [{ type: 'ellipsis', hidden: path.slice(0, path.length - maxSegments) }, ...asNodes(tail)];
  }
 ...
 */
export function collectCascaderSubtree<T>(index: CascaderIndex<T>, value: string): CascaderNode<T>[] {
...
export function getCascaderIndeterminate<T>(index: CascaderIndex<T>, selected: readonly string[]): Set<string> {
