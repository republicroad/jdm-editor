...
const ROW_GUTTER_CLASS = 'ps-[var(--cascader-row-inset,8px)]! pe-[calc(var(--cascader-row-inset,8px)*2_+_16px)]!';
...
const CascaderItem = React.memo(function CascaderItem({
  ...
  const hoverNavigates = expandTrigger === 'hover' && mode === 'columns' && as === 'option' && branch && !node.disabled;
  ...
  if (isCascaderMoreNode(node)) {
    const text = error ? labels.error : loading ? (node.count ? labels.loadingMore : labels.loading) : labels.loadMore;
  ...
  const shared = {
    ...
    className: cn(
      ...
      /* Three separate reasons to hand the check gutter back. A multi-select
         ...
         class-merge order. */
      leadingCheckbox || !showsIndicator || (branch && !branchesSelectable) ? ROW_FLUSH_CLASS : ROW_GUTTER_CLASS,
...
const CASCADER_SEPARATOR_CLASS = 'h-px bg-border my-1 mx-[calc(var(--cascader-list-pad,4px)*-1)]! shrink-0';
