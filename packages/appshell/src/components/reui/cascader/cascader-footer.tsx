 ...
 */
const FOOTER_SWALLOWED_KEYS = new Set(['Enter', ' ', 'ArrowUp', 'ArrowDown', 'Home', 'End', 'PageUp', 'PageDown']);
 ...
 */
function groupActionRuns(items: CascaderActionItem[]): { group?: string; items: CascaderActionItem[] }[] {
...
const CascaderSubmenuContext = React.createContext<CascaderSubmenuContextValue | undefined>(undefined);
 ...
 */
function CascaderSubmenu({ open: openProp, defaultOpen = false, onOpenChange, children }: CascaderSubmenuProps) {
...
type CascaderSubmenuTriggerClickEvent = Parameters<NonNullable<PopoverPrimitive.Trigger.Props['onClick']>>[0];
...
function CascaderSubmenuTrigger({
  ...
  // Base UI's merged ref prop intersects callback/ref/string unions that TS
...
type CascaderSubmenuKeyEvent = Parameters<NonNullable<PopoverPrimitive.Popup.Props['onKeyDown']>>[0];
...
function CascaderSubmenuContent({
  ...
  const handleKeyDown = React.useCallback(
    (event: CascaderSubmenuKeyEvent) => {
      ...
      const hit = ordered.find((item) => (item.textContent ?? '').trim().toLowerCase().startsWith(prefix));
      if (hit) {
        event.preventDefault();
        hit.focus();
      }
