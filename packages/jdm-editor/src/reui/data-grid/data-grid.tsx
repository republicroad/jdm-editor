...
import { type DataGridI18nConfig, type DataGridI18nOverrides, mergeDataGridI18n } from './data-grid-i18n';
 ...
 */
export type DataGridLayoutProps<TData extends object> = Omit<DataGridProps<TableFeatures, TData>, 'table' | 'children'>;
...
function createDataGridAutoSizeController<TData extends object>(
  ...
  return {
    apply(freeSpace: number) {
      ...
      const revert =
        state.applied && columnSizing[state.applied.columnId] === state.applied.grown ? state.applied : null;
...
export interface DataGridProps<TFeatures extends TableFeatures, TData extends object> {
   ...
   * not cover (transient highlight, active simulation row, …). Concatenated
   * into the body row's className on every render.
   */
 ...
 */
function useDataGrid<TData extends object = any>(): DataGridContextProps<TData> {
