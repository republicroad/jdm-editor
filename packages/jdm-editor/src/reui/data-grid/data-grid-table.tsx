...
import { Subscribe, flexRender } from '@tanstack/react-table';
...
function getPinningStyles<TData extends object>(column: Column<DataGridFeatures, TData, unknown>): CSSProperties {
...
function startDataGridColumnResizeOnEnd<TData extends object>(
  ...
  const resizeRtl = (table.options.columnResizeDirection === 'rtl') !== (column.getIsPinned() === 'end');
  const startOffset =
...
function DataGridTableAddRow() {
  ...
  return (
    <tr
      ...
      className={cn('hover:bg-muted/40 [thead+tbody_&:first-child]:border-t', props.tableClassNames?.rowCreate)}
    >
      <td
        // The extra column is the filler strip, rendered only when columns
        // are resizable.
        colSpan={table.getVisibleLeafColumns().length + (props.tableLayout?.columnsResizable ? 1 : 0)}
        className='p-0'
      >
        <Button
          type='button'
...
function DataGridTableBase({ children }: { children: ReactNode }) {
  ...
  const columnSizeVars = useMemo(() => {
    ...
    const fillColumnId = table.getVisibleLeafColumns().find((column) => column.columnDef.meta?.fillWidth)?.id;
  ...
  return (
    <table
      ...
      aria-multiselectable={cellSelectionOn && props.tableLayout?.cellSelectionMode !== 'single' ? true : undefined}
      ...
      aria-rowcount={cellSelectionOn ? Math.max(recordCount, table.getRowsInDisplayOrder().length) + 1 : undefined}
      className={cn(
...
function DataGridTableHeadRowCellResize<TData extends object>({
  ...
  const handleMouseDown = (event: ReactMouseEvent<HTMLElement>) => {
    ...
    stopResizeSessionRef.current?.();
    stopResizeSessionRef.current = startDataGridColumnResizeOnEnd(event, header, table, !isResizeModeOnEnd);
  };
...
function DataGridTableResizeIndicator({ viewportNodeRef }: { viewportNodeRef: RefObject<HTMLDivElement | null> }) {
  ...
  useLayoutEffect(() => {
    ...
    const directionMultiplier =
      (table.options.columnResizeDirection === 'rtl') !== (resizingHeader.column.getIsPinned() === 'end') ? -1 : 1;
    ...
    const viewportRoot = viewportElement ?? indicator.closest<HTMLElement>('[data-slot="data-grid-table-viewport"]');
    ...
    if (headerCellElement && viewportRoot) {
      ...
      const edgeClientX =
        (table.options.columnResizeDirection === 'rtl') !== (resizingHeader.column.getIsPinned() === 'end')
          ? anchorRect.left
          : anchorRect.right;
...
function DataGridTableBodyRow<TData extends object>({
  ...
  const rowClassName = props.getRowClassName?.(row.original, dataIndex);
...
function DataGridTableBodyRowExpandded<TData extends object>({ row }: { row: Row<DataGridFeatures, TData> }) {
  const { props, table } = useDataGrid();
  const expandedContent = table.getAllColumns().find((column) => column.columnDef.meta?.expandedContent)?.columnDef
...
function DataGridTableBodyRowCell<TData extends object>({
  ...
  const renderTd = (selection: ReturnType<typeof getDataGridCellSelectionCellAttrs> | null) => (
    <td
      ...
      // model.
      id={cellSelectionOn ? `${gridId}c-${toDataGridDomId(row.id)}-${toDataGridDomId(column.id)}` : undefined}
        ...
        selection
          ? (event) => {
              ...
              if (isDataGridCellInteractiveTarget(event.target)) {
                ...
                if (rangeSelectionOn && (event.shiftKey || event.metaKey || event.ctrlKey) && cell.getCanSelect()) {
                  event.preventDefault();
      ...
      onMouseEnter={rangeSelectionOn ? (event) => cell.getSelectionExtendHandler()(event) : undefined}
      className={cn(
        'align-middle',
        bodyCellSpacing,
        ...
        props.tableLayout?.cellBorder &&
          ...
          column.getIsLastColumn('center') &&
          'border-e-0',
        props.tableLayout?.columnsResizable && column.getCanResize() && !cellSelectionOn && 'truncate',
  ...
  return (
    <Subscribe source={table.atoms.cellSelection} selector={() => getDataGridCellSelectionCellAttrs(cell)}>
...
function DataGridTableRenderedRow<TData extends object>({
  ...
  const trailingSpacerSpan = centerWindow ? Math.max(0, centerVisibleCells.length - 1 - centerWindow.end) : 0;
  ...
  return (
    <Fragment>
      <DataGridTableBodyRow row={row} pinnedBoundary={pinnedBoundary} rowRef={rowRef} dataIndex={rowIndex}>
        ...
        {leadingSpacerSpan > 0 && (
          <td aria-hidden='true' data-slot='data-grid-table-virtual-col-spacer' colSpan={leadingSpacerSpan} />
        )}
        ...
        {trailingSpacerSpan > 0 && (
          <td aria-hidden='true' data-slot='data-grid-table-virtual-col-spacer' colSpan={trailingSpacerSpan} />
        )}
 ...
 */
function DataGridTableRowSelect<TData extends object>({ row }: { row: Row<DataGridFeatures, TData> }) {
  const { i18n } = useDataGrid();
  return (
...
function DataGridTableRowExpand<TData extends object>({
  ...
  return (
    <span
      ...
      className={cn('inline-flex shrink-0 items-center ps-(--data-grid-tree-padding) align-middle', className)}
    >
      {row.getCanExpand() ? (
        <button
          type='button'
...
function DataGridTableBodyRows<TData extends object>({ table }: { table: DataGridTableInstance<TData> }) {
...
const MemoizedDataGridTableBodyRows = memo(
  ...
  (_prev, next) => !!next.table.state.columnResizing.isResizingColumn || next.table._isSelectingCells === true,
...
function DataGridTable({ footerContent, renderHeader = true }: { footerContent?: ReactNode; renderHeader?: boolean }) {
  const { table, props } = useDataGrid();
  const mergedHeaderGroups = getDataGridTableMergedHeaderGroups(table);
  const hasRightPinnedColumns = hasDataGridTableRightPinnedColumns(table);
