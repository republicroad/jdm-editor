...
import type { CSSProperties, ComponentProps, KeyboardEvent as ReactKeyboardEvent, ReactNode } from 'react';
...
const CodeBlockDocumentContext = createContext<CodeBlockDocumentValue | null>(null);
 ...
 */
const CodeBlockSurfacePropsContext = createContext<CodeBlockSurfaceProps | null>(null);
...
function useCodeBlockConfig(part: string): CodeBlockConfigValue & { code: string } {
  const context = useContext(CodeBlockConfigContext);
  ...
  return useMemo(() => ({ ...context, code: document.code }), [context, document.code]);
}
...
function computeFoldRegions(lines: CodeBlockLine[]): CodeBlockFoldRegion[] {
  const indent = (text: string) => (INDENT_RE.exec(text)?.[0] ?? '').replace(/\t/g, '  ').length;
  ...
  while (stack.length) {
    const open = stack.pop()!;
    if (open.last > open.start) regions.push({ start: open.start, end: open.last });
  }
      ...
      {tokens.map((token, index) => {
        ...
        return (
          <span
            ...
            className={cn(token.word && 'bg-primary/10 ring-primary/20 rounded-sm px-0.5 py-px ring-1 ring-inset')}
            style={style}
          >
            {token.content}
          </span>
...
const CodeBlockLineRow = memo(function CodeBlockLineRow({
  ...
  return (
    <span
      ...
      onClick={selectable && onSelect ? (event) => onSelect(line.number, event.shiftKey) : undefined}
      className={cn(
        LINE_BASE_CLASS,
        LINE_NUMBER_CLASS,
        LINE_STATE_CLASS,
      ...
      {actions && active ? (
        <span
          ...
          className={cn(
            ...
            !inGutter && 'absolute end-2 top-0 flex h-(--code-block-line-height)',
            ...
            inGutter && 'absolute top-1/2 left-0 flex -translate-x-1/2 -translate-y-1/2 items-center justify-center',
          )}
        >
...
function CodeBlock({
  ...
  const plainLines = useMemo(() => toPlainLines(deferredSource, startLine), [deferredSource, startLine]);
  ...
  const lines = useMemo(() => {
    ...
    if (highlighted.spec !== specKey || !deferredSource.startsWith(highlighted.source)) {
      return plainLines;
    }
  ...
  }, [linesProp, shouldHighlight, highlighted, plainLines, deferredSource, specKey]);

  const focusMode = useMemo(() => lines.some((line) => line.state?.focused), [lines]);
  ...
  const [internalSelected, setInternalSelected] = useState<number[]>(defaultSelectedLines ?? []);
  ...
  const foldStarts = useMemo(() => new Map(foldRegions.map((region) => [region.start, region])), [foldRegions]);

  const [internalFolded, setInternalFolded] = useState<number[]>(defaultFolded ?? []);
  ...
  const actionsRegistry = useMemo(
    () => ({
      ...
      register: (owner: object, render: CodeBlockLineActionsRender | null, side: CodeBlockLineActionsSide = 'end') => {
        if (render) {
...
function CodeBlockSurface({
  ...
  useEffect(() => {
    if (wasStreaming.current && !streaming) {
      setAnnouncement(completeAnnouncement ?? `Code generation complete, ${lines.length} lines.`);
    }
  ...
  const toggleDisplayed = useCallback(
    (displayed: number, extend: boolean) => toggleLine(displayed - startLine + 1, extend),
  ...
  const handleKeyDown = useCallback(
    (event: ReactKeyboardEvent<HTMLElement>) => {
      ...
      if (event.key === 'Home') {
        ...
        const nextIndex =
          index === -1 ? (step === 1 ? 0 : lines.length - 1) : Math.min(lines.length - 1, Math.max(0, index + step));
        next = lines[nextIndex]?.number ?? startLine;
      }

      setActiveLine(next);
      const row = preRef.current?.querySelector<HTMLElement>(`[data-code-line="${next}"]`);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        aria-activedescendant={selectable && activeLine !== null ? `${contentId}-L${activeLine}` : undefined}
        tabIndex={0}
  ...
  return (
    <CodeBlockHeaderContext.Provider value={true}>
      <div
        ...
        className={cn('border-border flex min-h-9 shrink-0 items-center gap-2 border-b px-3 py-0 text-sm', className)}
        {...props}
      />
...
function CodeBlockTitle({ className, ...props }: ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn('text-muted-foreground min-w-0 truncate font-mono text-xs', className)}
      {...props}
    />
  );
}
 ...
 */
function CodeBlockLanguage({ className, children, ...props }: ComponentProps<'span'>) {
...
function CodeBlockCopyButton({
  ...
  const handleClick = useCallback(() => {
    ...
    const fallback = document_?.code || document_?.lines.map((line) => line.text).join('\n');
...
function CodeBlockExpandButton({
  ...
  const { expanded, setExpanded, collapsible, contentId } = useInternalConfig('CodeBlockExpandButton');
...
function CodeBlockDownloadButton({
  ...
  const handleClick = useCallback(() => {
    ...
    const extension = LANGUAGE_EXTENSIONS[context?.resolvedLanguage ?? ''] ?? 'txt';
  ...
  return (
    <Button
      ...
      className={cn(
        'shrink-0 [&_svg]:size-3.5',
        resolvedPosition === 'pinned' && 'absolute top-(--code-block-copy-top) right-(--code-block-copy-inset) z-20',
        resolvedPosition === 'pinned' &&
          !alwaysVisible &&
          'opacity-0 transition-opacity group-hover/code-block:opacity-100 focus-visible:opacity-100',
        className,
...
function CodeBlockLineActions({ children, side = 'end' }: CodeBlockLineActionsProps) {
...
function useCodeBlockFolding() {
  ...
  const foldedStarts = useMemo(() => [...folded].sort((a, b) => a - b), [folded]);
  const foldAll = useCallback(() => setFolded(foldRegions.map((region) => region.start)), [setFolded, foldRegions]);
...
function useCodeBlockSelection() {
  ...
  const selectedLines = useMemo(() => [...selected].sort((a, b) => a - b), [selected]);
...
export { ansiToLines, parseUnifiedDiff } from '#components/reui/code-block/code-block-highlight';
export type {
  CodeBlockDiffSpec,
