...
- 日期: 2026-09-17
- 状态: **ruling recorded —— 宿主裁决（2026-09-17）：三条贡献线由宿主手动执行，
  本仓后续开发不再考虑回馈上游，以自有路线为主**（「上游一般不接受这种改动，
  等以后有机会再推动」）。本文档 §2 降级为宿主手动执行的参考材料；
  §3 是本仓唯一有效的开发序列。
...
1. ~~三条贡献线的执行顺序~~ → **宿主手动执行，移出本仓开发序列**；素材（ADR-006/BP-08/
   troubleshooting §10/本文档 §1 校准表）保留自用。
2. N2 editor 升级窗口：与 editor 仓当前分支状态协调后执行。
3. v1.0 分叉日期（N3）：迁移与双包发版完成，剩余 blocker 为零，待宿主定日。
