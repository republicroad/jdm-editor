...
| `--seal-primary-color(-bg)` removed | Duplicates of `--seal-color-primary(-bg)` | Use `--seal-color-primary(-bg)` |
| Prefix sweep `grl-` → `seal-` (post-1.0, `reui` line) | Theme tokens, scope classes and component classes renamed together | Mechanically rename your `.grl-*` / `--grl-*` overrides — mapping in [`brand-sweep-codemod.md`](./design/brand-sweep-codemod.md) |
...
## `--seal-*` Variable Contract

All `--seal-*` variables are injected inline on the `.seal-root` container. The
following are **contract-stable** (not changing in the 1.x lifecycle):

All `--seal-color-*` palette tokens, `--seal-font-family`, `--seal-line-height`,
`--seal-border-radius`, `--seal-control-outline`, `--node-color-*`.

Previously-host-facing-only keys (`--seal-primary-color(-bg)`,
`--seal-color-primary-text-hover`, `--seal-color-info-text`, `--seal-color-bg-mask`)
